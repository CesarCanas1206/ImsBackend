...
class CollectionAssetController extends APIController
{
    /**
    ...
    public function assetList()
    {
        ...
        $results = Asset::with('assets')
            ->where(function ($where) {
                $where->where('parent_id', '')->orWhere('parent_id', null);
            })
            ->get()
            ->each(function (&$collection) {
                if (!empty($collection->assets)) {
                    $collection->assets->map([$this, 'mapFieldsToValues']);
                }
            });
    ...
    public function assetIdList($asset_id = '')
    {
        ...
        if (!empty($asset_id)) {
            ...
            $results = Asset::without(['fields', 'parent'])->get();
